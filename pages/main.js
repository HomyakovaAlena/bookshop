...
function createAddClassContentAppend(
  element,
  newClass,
  parentElement,
  content = ""
...
function header() {
  ...
  const wrapperHeader = createAddClassContentAppend(
    "div",
    "wrapperHeader",
    headerRow
  );

  const link = createAddClassContentAppend(
    "a",
    "linkLogoHeader",
    wrapperHeader
  );
  ...
  const title = createAddClassContentAppend(
    "h1",
    "title",
    logoDiv,
    "Wise Owl Bookshop"
  );
  const subtitle = createAddClassContentAppend(
    "h6",
    "subtitle",
    logoDiv,
    "Amazing Bookshop with home delivery"
  );
  const cart = createAddClassContentAppend(
    "span",
...
function createCard(books) {
  ...
  for (let i = 0; i < books.length; i++) {
    ...
    const divBookHeadings = createAddClassContentAppend(
      "div",
      "divBookHeadings",
      divBookRow
    );
    const headingTitle = createAddClassContentAppend(
      "h4",
      "headingTitle",
      divBookHeadings,
      books[i].title
    );
    const headingAuthor = createAddClassContentAppend(
      "h4",
      "headingAuthor",
      divBookHeadings,
      books[i].author
    ...
    const divBookPrice = createAddClassContentAppend(
      "div",
      "divBookPrice",
      divBuy
    );
    const headingPrice = createAddClassContentAppend(
      "h3",
      "headingPrice",
      divBookPrice,
      books[i].price + "$"
    );
    const buttonAdd = createAddClassContentAppend(
      "button",
      "btnAdd",
      divBuy,
      "+"
    ...
    const buttonShowMore = createAddClassContentAppend(
      "button",
      "btnShowMore",
      card,
      "Show more"
...
function cart() {
  const fragment = document.createDocumentFragment();
  const cartField = createAddClassContentAppend(
    "div",
    "cartField",
    fragment,
    ""
  ...
  const confirmOrder = createAddClassContentAppend(
    "a",
    "confirmOrderBtn",
    cartField,
    "Confirm order"
  ...
  function displayTotal() {
    ...
    //     cartField.visibility = 'hidden';
    // }
    // confirmOrder.disabled = false;
    //     confirmOrder.dispatchEvent(new CustomEvent("itemsUpdated"));
    // //   cartField.dispatchEvent(new CustomEvent("total0"));
    // } else {
    //     confirmOrder.removeEventListener("itemsUpdated", hideTotal);
    //     confirmOrder.removeEventListener("zeroTotal", hideTotal);
    // }
  ...
  // confirmOrder.addEventListener("itemsUpdated", hideTotal);
  // confirmOrder.addEventListener("zeroTotal", hideTotal);
...
function addModalShowMore() {
  const fragment = document.createDocumentFragment();
  const modalOuter = createAddClassContentAppend(
    "div",
    "modal-outer",
    fragment,
    ""
  );
  const modalInner = createAddClassContentAppend(
    "div",
    "modal-inner",
    modalOuter,
    ""
...
function showMoreModal() {
  ...
  // btnAddModal = modalInner.querySelector(".btnAdd");

  // links.forEach(link => link.addEventListener('click', handleCardButtonClick));
  ...
  <div class='modalRow'>
    ...
    const buttonAdd = modalInner.querySelectorAll(".btnAdd");
...
function dragAndDrop() {
  ...
  function handleDragStart(event) {
    ...
    const img = new Image();
    img.src = dragObj.src.replace("300", "100");
    console.log(img.src);
    ...
    // const card = this.currentTarget.closest('.card');
    // return card;
    // const cartField = document.querySelectorAll('.cartField');
  ...
  function handleDrop(event) {
    ...
    event.stopPropagation();
    console.log("drop");
    // dragObj.style.left = event.pageX - myX + 'px';
    // dragObj.style.top = event.pageY - myY + 'px';
    console.log(dragObj);

    console.log("Submitted!!!!");
    const button = dragObj;
    const card = dragObj.closest(".card");
    console.log(button, button.parentElement, card);
  ...
  cartField.addEventListener("drop", handleDrop);
}
...
function footer() {
  ...
  footer.id = "footer";
  const backToTop = createAddClassContentAppend(
    "a",
    "backToTop",
    footer,
    "Back to top"
  ...
  const spanFooter = createAddClassContentAppend(
    "span",
    "spanFooter",
    footerRow,
